...
describe('perfil y periodos de objetivos', () => {
  it('guarda el perfil local y conserva periodos históricos separados', async () => {
    ...
    expect(profile.dailyStepsGoal).toBe(10_000);
    const changed = await repository.saveProfile({ alias: 'Persona ficticia', age: 22, formulaSex: 'male', heightCm: 175, weightKg: 70, gymDaysPerWeek: 4, usualStepsPerDay: 8000, otherSportsPerWeek: 1, otherSportsDescription: 'Deporte ficticio', pal: 1.6, waterQuickAmountsMl: [300, 600], dailyStepsGoal: 12_000, consent: true });
    expect(changed.waterQuickAmountsMl).toEqual([300, 600]);
    expect(changed.dailyStepsGoal).toBe(12_000);
    ...
    await expect(repository.saveProfile({ alias: 'Persona ficticia', age: 22, formulaSex: 'male', heightCm: 175, weightKg: 70, gymDaysPerWeek: 4, usualStepsPerDay: 8000, otherSportsPerWeek: 1, otherSportsDescription: 'Deporte ficticio', pal: 1.6, dailyStepsGoal: 0, consent: true })).rejects.toThrow('objetivo diario de pasos');
