...
async function fixture() {
  ...
  const snapshot = { name: 'Alimento ficticio', energyKcal: 100, proteinG: 10, carbohydratesG: 8, fatG: null, baseUnit: 'g' as const, sourceUpdatedAt: now };
  const calculated = { energyKcal: 75, proteinG: 7.5, carbohydratesG: 6, fatG: null };
...
describe('backup completo y restauración temporal', () => {
  it('cifra, verifica, activa, revierte, reactiva y confirma sin borrar el anterior', async () => {
    ...
    expect((await database!.foods.where('datasetId').equals(prepared.candidateDatasetId).first())).toMatchObject({ dataOrigin: 'manual', energyKj: null, fatG: null });
