...
test.describe.serial('actualización real 0.2.1 → 0.3.1 bajo el mismo origen', () => {
  ...
  test('avisa, espera una fotografía pendiente y conserva las dos IndexedDB', async () => {
    ...
    await expect(page.getByRole('banner').getByText(/Versión 0\.3\.1/)).toBeVisible();
  ...
  test('reabre el build actualizado offline con esquema 6 y datos conservados', async ({ browserName }) => {
    ...
    try {
      ...
      await expect(offlinePage.getByRole('banner').getByText(/Versión 0\.3\.1/)).toBeVisible();
