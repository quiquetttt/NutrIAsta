...
import type { DetectionStatus, NutritionColumn, NutritionFieldKey, NutritionLabelResult, OcrProgress } from '@/ocr/nutrition-label-types';
import { foodRepository } from '@/storage/food-repository.web';

type Stage = 'capture' | 'processing' | 'result' | 'review';
type EditableValues = Record<NutritionFieldKey, string>;
...
const LABELS: Record<NutritionFieldKey, string> = {
  energyKj: 'Energía (kJ)', energyKcal: 'Energía (kcal)', fatG: 'Grasas (g)',
  carbohydratesG: 'Carbohidratos (g)', proteinG: 'Proteínas (g)',
};
...
export function LabelOcrFlow({ onCancel, onManual, onSaved }: {
  ...
  const warnings = useMemo(() => reviewWarnings(result, selectedColumn, values, portionAmount), [result, selectedColumn, values, portionAmount]);
  ...
  async function save() {
    ...
    try {
      ...
      const draft: FoodDraft = {
        ...
        proteinG: parseNumber(values.proteinG) * scale,
        carbohydratesG: parseNumber(values.carbohydratesG) * scale,
        fatG: parseNumber(values.fatG) * scale,
        energySource: 'declared', dataOrigin: 'label-photo',
        notes: 'Valores revisados manualmente tras OCR local.', favorite: false,
      };
      const portions = selectedColumn.basis === 'portion' ? [{ name: 'Porción de la etiqueta', amount: parseNumber(portionAmount) }] : [];
  ...
  if (stage === 'capture') return <View style={{ gap: 12 }}>{error ? <Notice danger text={error} /> : null}<LabelPhotoCapture busy={busy} onBusyChange={setBusy} onCancel={onCancel} onPrepared={(value) => void recognize(value)} /><ActionButton tone="secondary" label="Introducir manualmente" onPress={onManual} /></View>;
  ...
  return <View style={{ gap: 14 }}><Card>
    ...
    <ActionButton label="Guardar alimento" disabled={busy || !canSave(name, selectedColumn, values, portionAmount)} onPress={() => void prepareConfirmation()} /><ActionButton tone="secondary" label="Volver a fotografiar" disabled={busy} onPress={() => { setProcessed(null); setResult(null); setStage('capture'); }} /><ActionButton tone="secondary" label="Editar fotografía" disabled={busy} onPress={() => { setResult(null); setStage('capture'); }} /><ActionButton tone="secondary" label="Introducir manualmente" disabled={busy} onPress={onManual} /><ActionButton tone="secondary" label="Cancelar" disabled={busy} onPress={onCancel} />
...
function parseNumber(value: string) { return Number(value.trim().replace(',', '.')); }
function canSave(name: string, column: NutritionColumn, values: EditableValues, portionAmount: string) {
  if (!name.trim() || column.basis === 'unknown' || (column.basis === 'portion' && !(parseNumber(portionAmount) > 0))) return false;
  return ['energyKcal', 'proteinG', 'carbohydratesG', 'fatG'].every((key) => { const value = parseNumber(values[key as NutritionFieldKey]); return Number.isFinite(value) && value >= 0; });
}
function reviewWarnings(result: NutritionLabelResult | null, column: NutritionColumn | null, values: EditableValues, portionAmount: string) {
  if (!result || !column) return [];
  const warnings = [...result.warnings]; const kcal = parseNumber(values.energyKcal); const kj = parseNumber(values.energyKj);
  ...
  if (column.basis === 'portion' && !(parseNumber(portionAmount) > 0)) warnings.push('Falta una equivalencia válida para la porción.');
  if (column.basis === 'unknown') warnings.push('Selecciona una base nutricional clara antes de guardar.');
