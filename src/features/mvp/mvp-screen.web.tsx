...
export function MvpScreen() {
  ...
  async function refresh() {
    ...
    if (active) {
      ...
      if (nextProfile) {
        setProfileDraft({
          alias: nextProfile.alias, age: nextProfile.age, formulaSex: nextProfile.formulaSex,
          heightCm: nextProfile.heightCm, weightKg: nextProfile.weightKg,
          gymDaysPerWeek: nextProfile.gymDaysPerWeek, usualStepsPerDay: nextProfile.usualStepsPerDay,
          otherSportsPerWeek: nextProfile.otherSportsPerWeek, otherSportsDescription: nextProfile.otherSportsDescription,
          pal: nextProfile.pal, consent: true,
        });
      }
    }
  }
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: palette.background }} contentContainerStyle={{ alignItems: 'center', paddingHorizontal: 16, paddingTop: 20, paddingBottom: 64 }}>
      <View testID="mvp-content" style={{ width: '100%', maxWidth: 720, alignSelf: 'center', gap: 16 }}>
        ...
        {!profile ? (
          <ProfileEditor draft={profileDraft} setDraft={setProfileDraft} estimates={estimates} busy={busy} onboarding onSave={() => run('Perfil local guardado.', async () => { await profileRepository.saveProfile(profileDraft); setTab('profile'); })}/>
        ) : (
          <>
            ...
            {tab === 'today' ? <TodayEmpty target={targets.at(-1) ?? null}/> : tab === 'foods' ? <FoodCatalog/> : (
              <>
                ...
                <StorageStatusCard status={storage} onRequestPersistence={() => void run('Solicitud de persistencia completada.', async () => { await requestPersistentStorage(); })}/>
              </>
            )}
          </>
        )}
...
function ProfileEditor({ draft, setDraft, estimates, busy, onboarding = false, onSave }: { draft: ProfileDraft; setDraft: (value: ProfileDraft) => void; estimates: ReturnType<typeof estimatesForType>; busy: boolean; onboarding?: boolean; onSave: () => void }) {
  ...
  return <>
    ...
    <Orientation estimates={estimates} draft={draft}/>
  </>;
}

function Orientation({ estimates, draft }: { estimates: ReturnType<typeof estimatesForType>; draft: ProfileDraft }) {
  return <Card><SectionTitle eyebrow="Estimación">Orientación energética</SectionTitle>
    ...
    <Text selectable style={{ color: palette.muted }}>Ejemplos matemáticos: −5 % {Math.round(estimates.scenarios.deficit5)}, −10 % {Math.round(estimates.scenarios.deficit10)}, +5 % {Math.round(estimates.scenarios.surplus5)}, +10 % {Math.round(estimates.scenarios.surplus10)} kcal.</Text>
    ...
    <Text selectable style={{ color: palette.muted, fontSize: 13 }}>Entradas: {draft.weightKg} kg · {draft.heightCm} cm · {draft.age} años · PAL {draft.pal.toLocaleString('es-ES')}.</Text>
  </Card>;
}
...
function TabButton({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) { return <Pressable accessibilityRole="tab" accessibilityState={{ selected }} onPress={onPress} style={{ flex: 1, borderRadius: 14, padding: 13, backgroundColor: selected ? palette.navy : '#fff', borderWidth: 1, borderColor: selected ? palette.navy : palette.border }}><Text selectable style={{ textAlign: 'center', color: selected ? '#fff' : palette.navy, fontWeight: '800' }}>{label}</Text></Pressable>; }
