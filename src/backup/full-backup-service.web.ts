...
interface BuiltSnapshot { data: FullBackupData; dataJson: string; media: MediaValue[]; files: FullBackupFileDescriptor[]; counts: FullBackupManifest['entityCounts']; payloadBytes: number; fingerprint: string; }
interface DecodedFullBackup { manifest: FullBackupManifest; data: FullBackupData; media: MediaValue[]; payloadBytes: number; }

function emptyData(): FullBackupData { return Object.fromEntries(FULL_DATA_TABLES.map((table) => [table, []])) as unknown as FullBackupData; }
...
async function buildSnapshot(db: NutrIAstaMainDatabase, datasetId: string): Promise<BuiltSnapshot> {
  const data = emptyData(); const media: MediaValue[] = []; const files: FullBackupFileDescriptor[] = [];
  for (const tableName of FULL_DATA_TABLES) {
  ...
  const counts = Object.fromEntries(FULL_DATA_TABLES.map((table) => [table, data[table].length])) as FullBackupManifest['entityCounts'];
  const payloadBytes = files.reduce((sum, file) => sum + file.size, 0);
  if (payloadBytes > FULL_BACKUP_LIMITS.expandedBytes) throw new Error('El contenido local supera el límite del backup completo.');
...
export async function decodeFullBackup(file: File, password: string): Promise<DecodedFullBackup> {
  ...
  if (!/\.(nutriasta|zip)$/i.test(file.name) || file.size < 1 || file.size > FULL_BACKUP_LIMITS.archiveBytes) throw new Error('Selecciona un backup completo válido dentro del límite de tamaño.');
  ...
  try {
    ...
    for (const entry of all) {
      if (entry.directory || !entry.encrypted || entry.zipCrypto || !Number.isSafeInteger(entry.uncompressedSize) || entry.uncompressedSize < 0 || !Number.isSafeInteger(entry.compressedSize) || entry.compressedSize < 0 || entry.compressedSize > FULL_BACKUP_LIMITS.archiveBytes || entries.has(entry.filename)) throw new Error('El ZIP contiene una entrada no válida, duplicada o sin AES.');
      declared += entry.uncompressedSize; if (declared > FULL_BACKUP_LIMITS.expandedBytes + FULL_BACKUP_LIMITS.manifestBytes) throw new Error('El tamaño descomprimido declarado supera el límite.'); entries.set(entry.filename, entry);
    }
    const manifestText = await readText(requiredEntry(entries, FULL_BACKUP_MANIFEST_PATH), password, FULL_BACKUP_LIMITS.manifestBytes);
    ...
    const descriptor = dataDescriptor(manifest); const dataJson = await readText(requiredEntry(entries, descriptor.path), password, FULL_BACKUP_LIMITS.dataBytes);
    ...
    const data = parseFullBackupData(dataJson, manifest); const media: MediaValue[] = [];
    for (const photo of manifest.files.filter((item) => item.kind === 'photo')) {
      ...
      const [blob, thumb] = await Promise.all([readBlob(requiredEntry(entries, photo.path), password, FULL_BACKUP_LIMITS.mediaBytes), readBlob(requiredEntry(entries, thumbnail.path), password, FULL_BACKUP_LIMITS.thumbnailBytes)]);
    ...
    return { manifest, data, media, payloadBytes: manifest.files.reduce((sum, item) => sum + item.size, 0) };
...
export class FullBackupService {
  ...
  async status(): Promise<FullBackupStatus> {
    await this.repository.initialize(); const candidates = (await this.db.datasets.where('state').equals('staging').toArray()).filter((item) => item.source === 'format-2-backup').sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    ...
    return { lastBackupAt: ((await this.db.metadata.get(MAIN_META_KEYS.lastFullBackupAt))?.value as string | undefined) ?? null, prepared: candidate && run && manifest ? { candidateDatasetId: candidate.id, runId: run.id, previousDatasetId: await this.activeId(), manifest, payloadBytes: candidate.payloadBytes } : null, session: sessionRun?.sourceKind === 'format-2-backup' ? rawSession : null, blockedByOtherMigration: Boolean(rawSession && sessionRun?.sourceKind !== 'format-2-backup') };
  }
  private manifestFromDataset(dataset: MainDatasetMetadata): FullBackupManifest | null { if (!dataset.sourceBackupId || !dataset.sourceExportedAt || !dataset.entityCounts) return null; return { format: 'nutriasta-full-backup', formatVersion: 2, minimumAppVersion: FULL_BACKUP_MINIMUM_APP_VERSION, appVersion: APP_VERSION, backupId: dataset.sourceBackupId, sourceDatasetId: dataset.sourceDatasetId, exportedAt: dataset.sourceExportedAt, entityCounts: dataset.entityCounts as FullBackupManifest['entityCounts'], files: [], contentFingerprint: dataset.contentFingerprint }; }

  async create(password: string) {
    return trackUpdateBlockingOperation(async () => {
      assertFullBackupPassword(password); const datasetId = await this.activeId(); const built = await buildSnapshot(this.db, datasetId); const exportedAt = new Date().toISOString();
      const manifest: FullBackupManifest = { format: 'nutriasta-full-backup', formatVersion: 2, minimumAppVersion: FULL_BACKUP_MINIMUM_APP_VERSION, appVersion: APP_VERSION, backupId: createId('backup-full'), sourceDatasetId: datasetId, exportedAt, entityCounts: built.counts, files: built.files, contentFingerprint: built.fingerprint };
      parseFullBackupManifest(JSON.stringify(manifest)); const writer = new ZipWriter(new BlobWriter(FULL_BACKUP_MIME), { password, encryptionStrength: 3 });
      await writer.add(FULL_BACKUP_DATA_PATH, new TextReader(built.dataJson));
      ...
      await writer.add(FULL_BACKUP_MANIFEST_PATH, new TextReader(JSON.stringify(manifest))); const blob = await writer.close();
      if (blob.size > FULL_BACKUP_LIMITS.archiveBytes) throw new Error('El backup cifrado supera el límite permitido.');
  ...
  async prepare(file: File, password: string): Promise<PreparedFullRestore> {
    return trackUpdateBlockingOperation(async () => {
      ...
      const dataset: MainDatasetMetadata = { id, state: 'staging', source: 'format-2-backup', createdAt: now, updatedAt: now, recordCount: decoded.manifest.entityCounts.legacyViabilityRecords, photoCount: decoded.manifest.entityCounts.legacyViabilityPhotos + decoded.manifest.entityCounts.foodPhotos, payloadBytes: decoded.payloadBytes, sourceFingerprint: decoded.manifest.contentFingerprint, contentFingerprint: decoded.manifest.contentFingerprint, sourceDatasetId: decoded.manifest.sourceDatasetId, sourceBackupId: decoded.manifest.backupId, sourceExportedAt: decoded.manifest.exportedAt, entityCounts: decoded.manifest.entityCounts };
      const run: MigrationRun = { id: runId, state: 'staging', sourceKind: 'format-2-backup', sourceFingerprint: decoded.manifest.contentFingerprint, contentFingerprint: decoded.manifest.contentFingerprint, sourceDatasetId: decoded.manifest.sourceDatasetId, candidateDatasetId: id, createdAt: now, updatedAt: now };
      ...
      try {
        for (const tableName of FULL_DATA_TABLES) {
          ...
          const rows = decoded.data[tableName].map((row) => { const media = mediaById.get(String(row.id)); return media ? { ...row, datasetId: id, blob: media.blob, thumbnail: media.thumbnail } : { ...row, datasetId: id }; });
          for (let index = 0; index < rows.length; index += 100) await trackWrite(() => this.db.table(tableName).bulkPut(rows.slice(index, index + 100)));
        }
        const built = await buildSnapshot(this.db, id); if (built.fingerprint !== decoded.manifest.contentFingerprint || JSON.stringify(built.counts) !== JSON.stringify(decoded.manifest.entityCounts)) throw new Error('La verificación del dataset temporal ha fallado.');
  ...
  async activate(prepared: PreparedFullRestore) { return trackUpdateBlockingOperation(async () => { if (await this.activeId() !== prepared.previousDatasetId) throw new Error('El dataset activo cambió desde la preparación.'); const built = await buildSnapshot(this.db, prepared.candidateDatasetId); if (built.fingerprint !== prepared.manifest.contentFingerprint) throw new Error('El candidato ya no coincide con el backup.'); return this.repository.activateCandidate(prepared.candidateDatasetId); }); }
